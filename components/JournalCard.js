...
export default function JournalCard({ journal, index, isMe }) {
    ...
    return (
        <motion.div
            ...
            className={`p-5 md:p-8 rounded-[2rem] md:rounded-[2.5rem] glass-morphism border-white/5 relative group overflow-hidden ${isMe ? "bg-white/[0.02]" : "bg-purple-500/[0.03]"
                }`}
        >
            ...
            <div className="flex justify-between items-start mb-4 md:mb-6">
                ...
                <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-xl md:rounded-2xl bg-white/5 border border-white/5 text-xl md:text-2xl shadow-inner group-hover:scale-110 transition-transform duration-500">
                    {journal.mood}
                </div>
            </div>

            <div className="relative">
                <p className={`text-sm md:text-base leading-relaxed font-medium italic ${isMe ? "text-white/80" : "text-purple-100/80"
                    }`}>
                    "{journal.content}"
                </p>
            </div>
